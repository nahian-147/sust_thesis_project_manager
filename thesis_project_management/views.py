...
from .forms import TeamCreationForm, ArrangementParticipationForm, AssignSupervisorForm
from .models import Team, Arrangement
...
from .team_utility_functions import create_team_from_dict, get_arrangement_list_for_team, \
    ...
    get_all_teams_for_arrangement, add_participant_to_team
        ...
        if form.is_valid():
            ...
            if student.registration in student_list:
                team_info = {
                    ...
                    'project_thesis_title': form.cleaned_data.get('project_thesis_title'),
...
def assign_supervisor(request):
    ...
    if request.method == 'GET':
        ...
        form = AssignSupervisorForm(request.POST)
        if form.is_valid():
            supervisor_1 = form.cleaned_data['supervisor_1']
            s1_success = add_participant_to_team(team, supervisor_1)
            supervisor_2 = form.cleaned_data['supervisor_2']
            s2_success = add_participant_to_team(team, supervisor_2)
            supervisor_3 = form.cleaned_data['supervisor_3']
            s3_success = add_participant_to_team(team, supervisor_3)
